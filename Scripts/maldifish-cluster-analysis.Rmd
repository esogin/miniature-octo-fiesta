...
ggplot(tab, aes(x=as.factor(Var2), y=percent_of_group, fill=Var1)) + geom_bar(stat='identity', position='dodge')+ scale_fill_manual(values=c('mediumseagreen','lightcoral','skyblue')) + ggtitle('Percent of FISH Signals Across Clusters') + ylab('Percent of "FISH" Group') + xlab('Cluster Group') + guides(fill=guide_legend(title='FISH Group')) + theme_bw()
ggsave('Cluster Membership.eps')
```
