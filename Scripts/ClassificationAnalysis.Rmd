...
```{r}
msidata.subset.hm<-msidata.subset[,which(msidata.subset$Class %in% c('Green','Red'))]
...
```{r}
df<-spectra(msidata.subset.hm)[,]
rownames(df)<-msidata.subset.hm@imageData@dimnames[[1]] ## gets rownames
colnames(df)<-msidata.subset.hm@imageData@dimnames[[2]]
df.t<-as.data.frame(t(df))
int_matrix<-data.frame(Class=msidata.subset.hm$Class,df.t)
