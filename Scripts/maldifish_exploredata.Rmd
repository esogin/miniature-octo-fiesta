...
library(gplots)
library(gridExtra)
load('../Data/Cardinal_Processed_Data_withbkg.RData')
```
...
bkg<-names(mz.means[which(mz.means > trsh)])
length(bkg)
```
Compare through venn-diagram

```{r}
venn<-venn.diagram(x = list(mox,sox,host,soxmox, bkg), category.names = c('Mox','Sox','Host','Sox/Mox','Background'),filename = NULL, fill=c('Red','Green','Blue','Yellow', 'Gray'))
...
```{r}
metlist<-list(mox, sox, host, soxmox, bkg)
names(metlist)<-c('mox','sox','host','soxmox', 'background')
...
c<-data.frame(mz=inters$`mox:sox:host:soxmox:background`, label='common')
b<-data.frame(mz=inters$background, label='background')
sm<-data.frame(mz=inters$soxmox, label='soxmox')
vlists<-rbind(m,s,h,c,sm,b)
...
pca.scores.bkg<-as.data.frame(pca.mod[[1]]$scores[maldifishmz$Class=='Background',],Class='Background')
pca.scores.bkg$Class<-'background'

pca.scores<-rbind(pca.scores.green,pca.scores.mixed,pca.scores.red,pca.scores.tissue,pca.scores.bkg)
...
Re-Run PCA without background pixels 


```{r}
msidata<-maldifishmz[,!maldifishmz$Class %in% c('Background')]
set.seed(seed = 1)
pca.mod<-PCA(msidata,ncomp=4)
summary(pca.mod)
plot(summary(pca.mod))
```
...
```{r}
pca.scores.red<-as.data.frame(pca.mod[[1]]$scores[msidata$Class=='Red',])
pca.scores.red$Class<-'mox'

pca.scores.green<-as.data.frame(pca.mod[[1]]$scores[msidata$Class=='Green',],Class='sox')
pca.scores.green$Class<-'sox'

pca.scores.tissue<-as.data.frame(pca.mod[[1]]$scores[msidata$Class=='Tissue',],Class='host')
pca.scores.tissue$Class<-'host'

pca.scores.mixed<-as.data.frame(pca.mod[[1]]$scores[msidata$Class=='Mixed',],Class='mixed')
pca.scores.mixed$Class<-'sox&mox'
...
```{r}
msidata.host<-msidata[,msidata$Class=='Tissue']
pca.mod.2<-PCA(msidata.host,ncomp=2)
